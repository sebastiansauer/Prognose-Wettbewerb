...
#                          full.names = TRUE,
#                          pattern = "\\.R")
# invisible(map(funs_paths, source))


source("funs/build-paths.R")
source("funs/copy-submissions.R")
source("funs/add_rmse_col.R")
source("funs/build-main-df.R")
source("funs/add_solution_col.R")
source("funs/sanitize-pred-csv.R")
...
list(
  ...
  tar_target(notenliste_xslx,
             no_shows_added |>
               write_xlsx(path = paths$notenliste_output),
             packages = "writexl"),
  ...
  #              write_xlsx(paste0(paths$exam_root, paths$notenliste_temp_file)),
  #            packages = c("writexl"))
